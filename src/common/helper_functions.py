...
def collect_location_wise_count(days=300):
    ...
    data = pd.DataFrame(
        np.column_stack([company, location, email, date]),
        columns=["Company_Name", "Location", "Email", "Application_Date"],
    )
    # Change the format of application date and replace the none values by a year back date
    data["Application_Date"] = [
        pd.to_datetime(data["Application_Date"][i])
        if data["Application_Date"][i] != None
        else pd.to_datetime(datetime.now() - timedelta(days=365))
    ...
    index = [
        i
        for i in range(len(data))
        if (data["Application_Date"][i].value < start_date.value)
        & (data["Application_Date"][i].value > end_date.value)
    ]
    ...
    location_wise_df = pd.DataFrame(
        location_wise.size().reset_index(name="Group_Count")
    )
